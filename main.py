...
def main():
    ...
    ad_net = network.AdversarialNetwork(1024, 1024)
    ad_net = ad_net.cuda()
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # train for one epoch
        train(source_loader, target_loader, model, ad_net, criterion, optimizer, epoch, log_training, num_class)

        # evaluate on validation set
        if (epoch + 1) % args.eval_freq == 0 or epoch == args.epochs - 1:
            #prec1 = validate(val_loader, model, criterion, (epoch + 1) * len(target_loader), log_training)
            prec1 = validate(val_loader, model, criterion, num_class, (epoch + 1) * len(target_loader), log_training)
...
def train(source_loader, target_loader, model, ad_net, criterion, optimizer, epoch, log, num_class):
    ...
    for i, ((source_data, source_label), (target_data, target_label)) in data_loader:
        ...
        prec1, prec5 = accuracy(output_t.data, target_label, topk=(1,5))
        top1.update(prec1[0], target_input_var.size(0))
        top5.update(prec5[0], target_input_var.size(0))        
...
def validate(val_loader, model, criterion, num_class, iter, log):
    batch_time = AverageMeter()
    losses = AverageMeter()
    top1 = AverageMeter()
    top5 = AverageMeter()
    ...
    for i, (val_data, val_label) in enumerate(val_loader):
        val_label = val_label.cuda(async=True)
        val_input_var = torch.autograd.Variable(val_data, volatile=True)
        val_target_var = torch.autograd.Variable(val_label, volatile=True)

        # compute output
        output_v, feature_v = model(val_input_var)
        loss = criterion(output_v, val_target_var)

        # measure accuracy and record loss
        prec1, prec5 = accuracy(output_v.data, val_label, topk=(1,5))

        losses.update(loss.data[0], val_input_var.size(0))
        top1.update(prec1[0], val_input_var.size(0))
        top5.update(prec5[0], val_input_var.size(0))
